...
var files = {
    ...
    realmain: path.join(tmpdir, 'realmain.js'),
    lines: path.join(tmpdir, 'lines.txt'),
    reallines: path.join(tmpdir, 'reallines.txt'),
};

mkdirp.sync(tmpdir);
fs.writeFileSync(files.realmain, [
    'var fs = require("fs");',
    'var src = fs.readFileSync(__dirname + "/lines.txt", "utf8");',
    'console.log(src.toUpperCase());'
].join('\n'));

fs.writeFileSync(files.reallines, 'beep\nboop');

fs.symlinkSync(files.reallines, files.lines);
fs.symlinkSync(files.realmain, files.main);

test('api with brfs & symlinks', function (t) {
    t.plan(5);
    ...
    w.bundle(function (err, src) {
        ...
        setTimeout(function () {
            fs.writeFile(files.reallines, 'rObO-bOOgie', function (err) {
                t.ifError(err);
            });
        }, 1000);
    });
