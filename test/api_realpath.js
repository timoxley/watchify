...
var realfile = path.join(tmpdir, 'realmain.js');

mkdirp.sync(tmpdir);
fs.writeFileSync(realfile, 'console.log(555)');
fs.symlinkSync(realfile, file);

test('api with symlinks', function (t) {
    t.plan(5);
    ...
    w.bundle(function (err, src) {
        ...
        t.equal(run(src), '555\n');
        setTimeout(function () {
            fs.writeFile(realfile, 'console.log(333)', function (err) {
                t.ifError(err);
            });
        }, 1000);
    });
